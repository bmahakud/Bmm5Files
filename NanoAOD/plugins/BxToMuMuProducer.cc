...
struct KinematicFitResult{
  ...
  void postprocess(const reco::BeamSpot& beamSpot)
  {
    ...
    //std::cout<<"Secondary vertex Error matrix: ......."<<refitVertex->error().matrix()<<std::endl;
...
private:
  ...
  KinematicFitResult
  vertexMuonTracksWithKinematicFitter(const reco::Track* mutrk1,
                                 const reco::Track* mutrk2);
  ...
  edm::EDGetTokenT<reco::TrackCollection> trkVCToken_;
  ...
  //const edm::InputTag srcTrkVCTag_;
 // edm::EDGetTokenT<reco::TrackCollection> trkVCToken_;
...
trkVCToken_(consumes<reco::TrackCollection> (iConfig.getParameter<edm::InputTag>("srcTrkVC") ) ),
...
iteration_( iConfig.getParameter<std::string>( "iteration" ) ),
bdtReader0_("!Color:Silent"),
bdtReader1_("!Color:Silent"),
bdtReader2_("!Color:Silent")
//trkVCToken_(consumes<reco::TrackCollection> (iConfig.getParameter<edm::InputTag>("srcTrkVC")))
//srcTrkVCTag_(iConfig.getParameter<edm::InputTag>("srcTrkVC"))
{
    ...
    produces<float>("secVtxX");
    produces<float>("secVtxY");
    produces<float>("secVtxZ");
    
    produces<float>("secVtxXXer");
    produces<float>("secVtxYYer");
    produces<float>("secVtxZZer");

    produces<float>("secVtxXYer");
    produces<float>("secVtxYZer");
    produces<float>("secVtxZXer");
    ...
    //trkVCToken_ = consumes<reco::TrackCollection>(edm::InputTag(srcTrkVCTag_));
			       ...
			       ) 
{
   ...
   if(iteration=="iteration2" ){//iteration 2
     try{ 


 
      auto kinematicMuMuVertexFit = vertexMuonTracksWithKinematicFitter(muTrk1, muTrk2);
      kinematicMuMuVertexFit.postprocess(*beamSpot_);  



    //std::cout<<"itr2Event: "<<iEvent.id().event()<<"--fillMuMuInfo: muTrk1 pt "<<muTrk1->pt()<<std::endl;
    //std::cout<<"itr2Event: "<<iEvent.id().event()<<"--fillMuMuInfo: muTrk2 pt "<<muTrk2->pt()<<std::endl;   

    //std::cout<<"itr2Event: "<<iEvent.id().event()<<"fillMuMuInfo: muon1 pt"<<muon1.innerTrack().get()->pt()<<std::endl;
    //std::cout<<"itr2Event: "<<iEvent.id().event()<<"fillMuMuInfo: muon2 pt"<<muon2.innerTrack().get()->pt()<<std::endl;



    auto SVx = kinematicMuMuVertexFit.refitVertex->position().x();
    
    auto SVy = kinematicMuMuVertexFit.refitVertex->position().y();
    auto SVz = kinematicMuMuVertexFit.refitVertex->position().z();
    auto SVerxx = kinematicMuMuVertexFit.refitVertex->error().matrix()(0,0);
    auto SVeryy = kinematicMuMuVertexFit.refitVertex->error().matrix()(1,1);
    auto SVerzz = kinematicMuMuVertexFit.refitVertex->error().matrix()(2,2);

    auto SVerxy = kinematicMuMuVertexFit.refitVertex->error().matrix()(1,0);
    auto SVerxz = kinematicMuMuVertexFit.refitVertex->error().matrix()(2,0);
    auto SVeryz = kinematicMuMuVertexFit.refitVertex->error().matrix()(1,2);

    //std::cout<<"itr2Event: "<<iEvent.id().event()<<"--fillMuMuInfo: SV info:   "<<SVx<<", "<<SVy<<", "<<SVz<<", "<<SVerxx<<", "<<SVeryy<<", "<<SVerzz<<", "<<SVerxy<<", "<<SVeryz<<", "<<SVerxz<<std::endl;

    dimuonCand.addUserFloat("SVx", SVx);
    dimuonCand.addUserFloat("SVy", SVy);
    dimuonCand.addUserFloat("SVz", SVz);
    dimuonCand.addUserFloat("SVerxx", SVerxx);
    dimuonCand.addUserFloat("SVeryy", SVeryy);
    dimuonCand.addUserFloat("SVerzz", SVerzz);
    dimuonCand.addUserFloat("SVerxy", SVerxy);
    dimuonCand.addUserFloat("SVerxz", SVerxz);
    dimuonCand.addUserFloat("SVeryz", SVeryz);
  ...
  return kinematicMuMuVertexFit;
   ...
   //   std::cout<<"running iteration2, kin mass"<<kinematicMuMuVertexFit.mass()<<std::endl;
   ...
   //iteration2 block of codes are complete


   //following code is for iteration1 and for cases where iteration2 is not valid
  ...
  // std::cout<<"itr1Event_fillMuMuInfo: "<<iEvent.id().event()<<"--fillMuMuInfo: SV info:   "<<SVx<<", "<<SVy<<", "<<SVz<<", "<<SVerxx<<", "<<SVeryy<<", "<<SVerzz<<", "<<SVerxy<<", "<<SVeryz<<", "<<SVerxz<<std::endl;

    // std::cout<<"SV information inside filMuMuInfo: "<<SVx<<", "<<SVy<<", "<<SVz<<"  "<<std::endl;
...
void BxToMuMuProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
     ...
     auto secVtxX = std::make_unique<float>();
     auto secVtxY = std::make_unique<float>();
     auto secVtxZ = std::make_unique<float>();
     auto secVtxXXer = std::make_unique<float>();
     auto secVtxYYer = std::make_unique<float>();
     auto secVtxZZer = std::make_unique<float>();
     auto secVtxXYer = std::make_unique<float>();
     auto secVtxYZer = std::make_unique<float>();
     auto secVtxZXer = std::make_unique<float>();
    ...
    edm::Handle<reco::TrackCollection> tracksVC;

    if(iteration_=="iteration2"){
    iEvent.getByToken(trkVCToken_, tracksVC);
    }
    ...
    //std::cout<<"Num Muons: "<<nMuons<<std::endl;
	...
	for (unsigned int j = 0; j < good_muon_candidates.size(); ++j) {
	  ...
	  // Kinematic Fits
          ...
          //try to get the inner tracks of the muon1 and muon2 and then do a vertex fit and pass to the kinematic mumu vertex fit. Or you can use the returned tracks
          //of kinematicMuMuVertexFit to do the secondaryVertexRefit.

          
          reco::TrackRef trackRef1=muon1.innerTrack();
          reco::TrackRef trackRef2=muon2.innerTrack();

          
          try{
             InnerMuons->push_back(*trackRef1);
             InnerMuons->push_back(*trackRef2);
             }
          catch(...){}
          ...
          //std::cout<<"refitted track size: "<<iEvent.id().event()<<" ... "<<tracksVC->size()<<std::endl;


 
          if (tracksVC.isValid() && iteration_=="iteration2"){

          // std::cout<<"refitting tracks"<<std::endl;
           int iterator =0;
           for(reco::TrackCollection::const_iterator itTrack = tracksVC->begin();itTrack != tracksVC->end();++itTrack) {
                  // std::cout<<"Event: "<<iEvent.id().event()<<"  trackpt inside loop:producefunction--->"<<itTrack->pt()<<std::endl;
                 iterator=iterator+1;
                 if(iterator==1){
                  reco::Track mTrk1=*itTrack;
                  muTrk1=&mTrk1;
                                }

                if(iterator==2){
                   reco::Track mTrk2=*itTrack;
                   muTrk2=&mTrk2;
                              }
                }
           }
	  ...
	  auto kinematicMuMuVertexFit = fillMuMuInfo(dimuonCand,iEvent,muon1,muon2,muTrk1,muTrk2,iteration_);
            
          //std::cout<<"Event: "<<iEvent.id().event()<<std::endl;
          //if(iteration_=="iteration1")std::cout<<"---eventno:  "<< iEvent.id().event()<<" muon 1 pt: "<<muon1.innerTrack().get()->pt()<<std::endl;
          //if(iteration_=="iteration2")std::cout<<"---eventno:  "<<iEvent.id().event()<<"  muon 1 pt: "<<muTrk1->pt()<<std::endl;

          //if(iteration_=="iteration1")std::cout<<"---eventno:  "<< iEvent.id().event()<<" muon 2 pt: "<<muon2.innerTrack().get()->pt()<<std::endl;
          //if(iteration_=="iteration2")std::cout<<"---eventno:  "<<iEvent.id().event()<<"  muon 2 pt: "<<muTrk2->pt()<<std::endl;

 
          //if(iteration_=="iteration1")std::cout<<"---eventno:  "<<iEvent.id().event()<<"--dimuon mass"<<dimuonCand.userFloat("kin_mass")<<std::endl;
         // if(iteration_=="iteration2")std::cout<<"---eventno:  "<<iEvent.id().event()<<"--dimuon mass"<<dimuonCand.userFloat("kin_mass")<<std::endl;

          //std::cout<<"Event No: "<<iEvent.id().event()<<"inside produce funct: "<<dimuonCand.userFloat("SVx")<<" "<<dimuonCand.userFloat("SVy")<<" "<<dimuonCand.userFloat("SVz")<<" "<<dimuonCand.userFloat("SVerxx")<<" "<<dimuonCand.userFloat("SVeryy")<<" "<<dimuonCand.userFloat("SVerzz")<<" "<<dimuonCand.userFloat("SVerxy")<<" "<<dimuonCand.userFloat("SVeryz")<<" "<<dimuonCand.userFloat("SVerxz")<<std::endl;

        // if(iteration_=="iteration1")std::cout<<"itr1 mumumass-- "<<iEvent.id().event()<<"--"<<dimuonCand.userFloat("kin_mass")<<std::endl;
        // if(iteration_=="iteration2")std::cout<<"itr2 mumumass-- "<<iEvent.id().event()<<"--"<<dimuonCand.userFloat("kin_mass")<<std::endl;



         auto secVtxX_ = std::make_unique<float>(dimuonCand.userFloat("SVx"));
         secVtxX = std::move(secVtxX_);

         auto  secVtxY_ = std::make_unique<float>(dimuonCand.userFloat("SVy"));
         secVtxY = std::move(secVtxY_);

         auto  secVtxZ_ = std::make_unique<float>(dimuonCand.userFloat("SVz"));
         secVtxZ = std::move(secVtxZ_);


         auto  secVtxXXer_ = std::make_unique<float>(dimuonCand.userFloat("SVerxx"));
         secVtxXXer = std::move(secVtxXXer_);

         auto  secVtxYYer_ = std::make_unique<float>(dimuonCand.userFloat("SVeryy"));
         secVtxYYer = std::move(secVtxYYer_);

         auto  secVtxZZer_ = std::make_unique<float>(dimuonCand.userFloat("SVerzz"));
         secVtxZZer = std::move(secVtxZZer_);


         auto  secVtxXYer_ = std::make_unique<float>(dimuonCand.userFloat("SVerxy"));
         secVtxXYer = std::move(secVtxXYer_);

         auto  secVtxYZer_ = std::make_unique<float>(dimuonCand.userFloat("SVeryz"));
         secVtxYZer = std::move(secVtxYZer_);


         auto  secVtxZXer_ = std::make_unique<float>(dimuonCand.userFloat("SVerxz"));
         secVtxZXer = std::move(secVtxZXer_);
    ...
    //std::cout<<"SVz before writing to event:  "<<secVtxZ<<std::endl;

    //if(iteration_=="iteration1"){
    iEvent.put(std::move(InnerMuons),"innerTrackerMuons"); 
    iEvent.put(std::move(secVtxX),"secVtxX");
    iEvent.put(std::move(secVtxY),"secVtxY");
    iEvent.put(std::move(secVtxZ),"secVtxZ");

    iEvent.put(std::move(secVtxXXer),"secVtxXXer"); 
    iEvent.put(std::move(secVtxYYer),"secVtxYYer");
    iEvent.put(std::move(secVtxZZer),"secVtxZZer");

    iEvent.put(std::move(secVtxXYer),"secVtxXYer");
    iEvent.put(std::move(secVtxYZer),"secVtxYZer");
    iEvent.put(std::move(secVtxZXer),"secVtxZXer");
    //}
...
KinematicFitResult BxToMuMuProducer::vertexMuonTracksWithKinematicFitter(const reco::Track *muTrk1,const reco::Track *muTrk2){

  std::vector<const reco::Track*> trks;
  std::vector<float> masses;
  trks.push_back(muTrk1);
  masses.push_back(MuonMass_);
  trks.push_back(muTrk2);
  masses.push_back(MuonMass_);
  return vertexWithKinematicFitter(trks,masses);
...
void
BxToMuMuProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {


  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("srcTrkVC", edm::InputTag("myRefittedTracks"));
  desc.setUnknown();
